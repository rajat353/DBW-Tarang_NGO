...
{
$sql = "SELECT * from campmembers WHERE emailid='$x' AND passwrd='$password1'";
$result = $conn->query($sql);
 ...
 else
 {
	...
	$sql="UPDATE campmembers set passwrd='$password2' where emailid='$x'";
	if($conn->query($sql))
	{
...
{
	$sql = "SELECT * from oladagemembers WHERE emailid='$x' AND passwrd='$password1'";
$result = $conn->query($sql);
 ...
 else
 {
	...
	$sql="UPDATE oldagemembers set passwrd='$password2' where emailid='$x'";
	if($conn->query($sql))
	{
		echo "<script>alert('PASSWORD UPDATED SUCCESSFULLY'); </script>";
		header("Location:tarang.php");
